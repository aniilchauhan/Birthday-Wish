  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      try {
        ...
        res.status(400).json({ error: 'Config is not JSON-serializable' });
        return;
      }
      ...
      for (let attempt = 0; attempt < 6; attempt++) {
        ...
        try {
          ...
          console.error('Surprise create failed:', err);
          res.status(500).json({ error: 'Could not save surprise' });
          return;
        }
      }
    ...
    console.error('api/surprise unhandled:', e);
    if (!res.headersSent) {
